...
public:
    ...
    /// \brief 获取皮肤列表迭代器的起始点
    SkinIter begin() const {
        return m_skins.begin();
    }
    ...
    /// \brief 获取已被选用的皮肤
    SkinInfo *GetSelectedSkin() const {
        return m_selected;
    }

    /// \brief 设置已被选用的皮肤
    void SetSelectedSkin(SkinInfo *skin) {
        m_selected = skin;
    }
...
public:
    ...
    public:
        ...
        ///
        /// 路径末尾已加上“/”。
        wxString rootPath() const {
            return m_strRootPath;
        }

        /// \brief 获取皮肤作者
        wxString author() const {
            return m_author;
        }
        ...
        /// \brief 获取皮肤掩码色
        wxColour maskColor() const {
            return m_maskColor;
        }

        /// \brief 当前皮肤缩略图是否已然被读入
        bool isThumbNailOk() const {
            return m_bmpThumbnail.IsOk();
        }
