...
public:
    ...
    /// \brief 设置歌曲的播放次数
    OOPListEntry &playCount(int c) {
        m_playCount = c;
        return *this;
    }

    /// \brief 将歌曲的播放次数递增1
    OOPListEntry &playCountPlus() {
        m_playCount++;
        return *this;
    }
    ...
    /// \brief 设置歌曲路径名
    OOPListEntry &path(const wxString &s) {
        m_path = s;
        return *this;
    }
...
public:
    ...
    /// \brief 设置歌曲的播放次数
    OOPSong &playCount(int c) {
        m_listEntry->playCount(c);
        return *this;
    }

    /// \brief 将歌曲的播放次数递增1
    OOPSong &playCountPlus() {
        m_listEntry->playCountPlus();
        return *this;
    }

    /// \brief 获取歌曲的播放次数
    int playCount() const {
        return m_listEntry->playCount();
    }

    /// \brief 设置歌曲加入列表的时间
    OOPSong &addTime(long t) {
        m_listEntry->addTime(t);
        return *this;
    }

    /// \brief 获取歌曲加入列表的时间
    long addTime() const {
        return m_listEntry->addTime();
    }
    ...
    /// \brief 设置歌曲名
    OOPSong &title(const wxString &s) {
        m_title = s;
        return *this;
    }

    /// \brief 获取歌手名
    wxString artist() const {
        return m_artist;
    }

    /// \brief 设置歌手名
    OOPSong &artist(const wxString &s) {
        m_artist = s;
        return *this;
    }
    ...
    /// \brief 获取歌曲路径
    wxString path() const {
        return m_listEntry->path();
    }

    /// \brief 设置歌曲路径名
    OOPSong &path(const wxString &s) {
        m_listEntry->path(s);
        return *this;
    }
    ...
    OOPSong &lrcPath(const wxString &s) {
        m_lrcPath = s;
        return *this;
    }

    /// \brief 获取歌曲所属专辑
    wxString album() const {
        return m_album;
    }

    /// \brief 设置歌曲所属专辑
    OOPSong &album(const wxString &s) {
        m_album = s;
        return *this;
    }

    /// \brief 获取歌曲所属流派
    wxString genre() const {
        return m_genre;
    }

    /// \brief 设置歌曲所属流派
    OOPSong &genre(const wxString &s) {
        m_genre = s;
        return *this;
    }
    ...
    /// \brief 设置歌曲的备注信息
    OOPSong &comment(const wxString &s) {
        m_comment = s;
        return *this;
    }
    ...
    /// \brief 获取歌曲长度（量纲：s[秒]）
    int length() const {
        return m_listEntry->length();
    }

    /// \brief 设置歌曲长度（量纲：s[秒]）
    OOPSong &length(int l) {
        m_listEntry->length(l);
        return *this;
    }
