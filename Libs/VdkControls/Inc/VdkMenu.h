...
public:
    ...
    */
    ...
    bool IsInitialized() const {
        return TestState(VMST_INITIALIZED);
    }
...
public:
    ...
    /// \brief 返回菜单项常量迭代器的起始
    ItemIter_Const begin() const {
        return m_items.begin();
    }

    /// \brief 返回菜单项常量迭代器的终止
    ItemIter_Const end() const {
        return m_items.end();
    }
...
public:
    ...
    int GetBitmapRegionWidth() const {
        return m_sstyle->bitmapRegion;
    }

    /// \brief 获取菜单项高
    int GetRowHeight() const {
        return m_sstyle->rowHeight;
    }
    ...
    int GetBorderWeight() const {
        return m_sstyle->borderWeight;
    }

    /// \brief 获取子菜单的右边箭头宽度
    int GetRightArrowSize() const {
        return m_sstyle->rightArrowSize;
    }
    ...
    /// \brief 获取高亮项所用的画刷
    const wxBrush &GetHilightBrush() const {
        return m_sstyle->hilightBrush;
    }

    /// \brief 获取选中项普通状态下绘制时所用的画刷
    const wxBrush &GetCheckedBrush() const {
        return m_sstyle->checkedBrush;
    }
    ...
    /// \brief 获取菜单项高亮时的边框颜色
    wxPen GetHilightBorderPen() const {
        return m_sstyle->hilightBorderPen;
    }

    /// \brief 获取菜单边框颜色
    wxPen GetBorderPen() const {
        return m_sstyle->borderPen;
    }

    /// \brief 获取子菜单与父菜单之间的间隙宽度
    int GetSubMainGap() const {
        return m_sstyle->subMainGap;
    }
...
public:
    ...
    /// \brief 获取菜单项加速键描述文本
    const wxString &accelString() const {
        return strAccel;
    }
    ...
    int accelStringWidth() {
        return nAcceStrWidth;
    }

    /// \brief 是否设置了左侧位图
    bool isBitmapOk() const {
        return BitmapId.IsOk();
    }

    /// \brief 获取位图 ID
    const VdkBitmapArrayId &bmpArrayID() const {
        return BitmapId;
    }
    ...
    /// \brief 获取子菜单句柄
    VdkMenu *subMenu() const {
        return pSubMenu;
    }

    /// \brief 设置子菜单句柄
    VdkMenuItem &subMenu(VdkMenu *sm) {
        pSubMenu = sm;
        return *this;
    }
    ...
    bool checkable() const  {
        return TestStyle(VMIS_CHECKABLE);
    }
    ...
    /// \brief 测试菜单项当前是否已被选中
    bool checked() const  {
        return TestState(VMIST_CHECKED);
    }
    ...
    /// \brief 测试菜单项是否已被禁用
    bool disabled() const {
        return TestState(VMIST_DISABLED);
    }
...
inline VdkMenuItem *NewMenuItem() {
    return new VdkMenuItem();
}
