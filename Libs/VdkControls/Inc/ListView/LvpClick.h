...
public:
    ...
    /// 存取选中项的背景画刷
    wxBrush GetSelectedBrush() const {
        return m_selected;
    }
    void SetSelectedBrush(const wxBrush &brush) {
        m_selected = brush;
    }
...
public:
    ...
    /// 获取被选中的行
    const ArrayOfSortedInts &GetSelItems() const {
        return m_selItems;
    }
...
protected:
    ...
    /// 获取最后点击的选项
    int &GetVeryLastClicked() {
        return m_veryLastClicked;
    }

    /// 获取被选中的行
    ArrayOfSortedInts &GetSelItems() {
        return m_selItems;
    }
