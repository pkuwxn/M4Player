...
public:
    ...
    void ReverseStyle(long style) {
        m_style ^= style;
    }

    /// \brief 测试当前风格属性集中是否设置有指定属性\a style
    bool TestStyle(long style) const {
    ...
    /// 设置即返回真。
    bool TestState(long state) const {
