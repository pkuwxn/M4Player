...
public:
    ...
    T &window(VdkWindow *w) {
        Win = w;
        return static_cast<T &>(*this);
    }

    /// \brief 设置父控件
    T &parent(VdkControl *p) {
        Parent = p;
        return static_cast<T &>(*this);
    }

    /// \brief 控件初始化后是否直接添加到所属的 VdkWindow 的控件列表中
    T &addToWindow(bool b) {
        AddToWindow = b;
        return static_cast<T &>(*this);
    }

    /// \brief 设置控件的名字
    T &name(const wxString &s) {
        Name = s;
        return static_cast<T &>(*this);
    }
    ...
    /// \brief 设置控件所用的字体
    T &font(const wxFont &f) {
        Font = f;
        return static_cast<T &>(*this);
    }

    /// \brief 设置控件相对于父控件的对齐属性
    T &align(align_type a) {
        Align = a;
        return static_cast<T &>(*this);
    }
...
public:
    ...
    static const wxColour &GetMaskColour() {
        return ms_maskColor;
    }

    /// \brief 设置当前进程空间的控件皮肤位图的透明掩码色
    static void SetMaskColour(const wxColour &color) {
        ms_maskColor = color;
    }

    /// \brief 获取当前进程空间的控件皮肤文件根目录
    static const wxString &GetRootPath() {
        return ms_rootPath;
    }
...
public:
    ...
    wxPoint GetPosition() const {
        return wxPoint(m_Rect.x, m_Rect.y);
    }
    ...
    /// \see GetRect
    wxRect GetAbsoluteRect() const {
        return TranslateRect(m_Rect);
    }
    ...
    ///
    /// 当没有父控件时，对其属性是相对于所属 VdkWindow 而言的。
    void SetAlign(const align_type &align) {
        m_align = align;
    }
    ...
    /// \brief 获取子控件链表的前一个结点
    VdkControl *GetPrev() const {
        return m_prev;
    }

    /// \brief 设置子控件链表的前一个结点
    void SetPrev(VdkControl *prev) {
        m_prev = prev;
    }

    /// \brief 获取子控件链表的后一个结点
    VdkControl *GetNext() const {
        return m_next;
    }

    /// \brief 设置子控件链表的后一个结点
    void SetNext(VdkControl *next) {
        m_next = next;
    }

    /// \brief 获取子控件链表的第一个结点
    VdkControl *GetFirstChild() const {
        return m_firstChild;
    }
    ...
    bool IsCtrlHandler() const {
        return TestStyle(VCS_HANDLER);
    }

    /// \brief 是否适合激发回调函数
    bool IsReadyForEvent() const {
        return m_id != wxID_ANY;
    }
