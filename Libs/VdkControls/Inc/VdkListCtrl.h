...
struct VdkLcColumn {
    ...
    /// 重新计算溢出，而是在绘制时才即时修改。
    int GetDrawingVersion() const {
        return m_drawingVersion;
    }

    /// \brief 提高当前绘图版本
    void IncreaseDrawingVersion() {
        m_drawingVersion++;
    }
...
public:
    ...
    ///
    /// 成功插入一列后，这列的序号就是\a i
    VdkLcColumnInitializer &index(int i) {
        Index = i;
        return *this;
    }
    ...
    /// \brief 设置列的初始列宽
    VdkLcColumnInitializer &width(int w) {
        Width = w;
        return *this;
    }
...
protected:
    ...
    int GetLastDraggTarget() const {
        return m_lastDraggTarget;
    }
    ...
    void SetShownItemsAddIn(int addin) {
        m_nShownItemsAddIn = addin;
    }

    /// \brief 获取在正常的垂直显示范围之下还要显示的项目数
    int GetShownItemsAddin() const {
        return m_nShownItemsAddIn;
    }
