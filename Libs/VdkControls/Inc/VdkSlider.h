...
public:
    ...
    /// \brief 设置滚动条的滚动方向
    Slider &vertical(bool b) {
        Vertical = b;
        return *this;
    }

    /// \brief 设置滚动条手柄所用位图的分割份数
    Slider &thumbExpl(int nExpl) {
        Expl = nExpl;
        return *this;
    }

    /// \brief 设置滚动条手柄所用位图的拉伸叠放长度
    Slider &thumbTileLen(int len) {
        ThumbTileLen = len;
        return *this;
    }
...
public:
    ...
    const VdkButton *GetThumb() const {
        return m_pThumb;
    }
...
private:
    ...
    struct SliderImp {
        ...
        /// \brief 释放鼠标指针
        void Release() {
            ptr = INVALID_MOUSE_ON_THUMB;
        }

        /// \brief 鼠标指针是否位于手柄上，这时允许拖动
        bool IsOk() const {
            return ptr != INVALID_MOUSE_ON_THUMB;
        }
    };
