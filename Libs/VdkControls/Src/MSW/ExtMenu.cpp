    ...
    while (code == HC_ACTION) {
        ...
        if (hMenu) {
            ...
            if (dwStyle & WS_MINIMIZE) {
                dprintf(L"WS_MINIMIZE");
            }
            if (dwStyle & WS_VISIBLE) {
                dprintf(L"WS_VISIBLE");
            }
            if (dwStyle & WS_DISABLED) {
                dprintf(L"WS_DISABLED");
            }
            if (dwStyle & WS_CLIPSIBLINGS) {
                dprintf(L"WS_CLIPSIBLINGS");
            }
            if (dwStyle & WS_CLIPCHILDREN) {
                dprintf(L"WS_CLIPCHILDREN");
            }
            if (dwStyle & WS_MAXIMIZE) {
                dprintf(L"WS_MAXIMIZE");
            }
            /*
            ...
            */
            ...
            if (dwStyle & WS_DLGFRAME) {
                dprintf(L"WS_DLGFRAME");
            }
            if (dwStyle & WS_VSCROLL) {
                dprintf(L"WS_VSCROLL");
            }
            if (dwStyle & WS_HSCROLL) {
                dprintf(L"WS_HSCROLL");
            }
            if (dwStyle & WS_SYSMENU) {
                dprintf(L"WS_SYSMENU");
            }
            if (dwStyle & WS_THICKFRAME) {
                dprintf(L"WS_THICKFRAME");
            }
            if (dwStyle & WS_GROUP) {
                dprintf(L"WS_GROUP");
            }
            if (dwStyle & WS_TABSTOP) {
                dprintf(L"WS_TABSTOP");
            }
            if (dwStyle & WS_MINIMIZEBOX) {
                dprintf(L"WS_MINIMIZEBOX");
            }
            if (dwStyle & WS_MAXIMIZEBOX) {
                dprintf(L"WS_MAXIMIZEBOX");
            }
            ...
            if (dwExtStyle & WS_EX_DLGMODALFRAME) {
                dprintf(L"WS_EX_DLGMODALFRAME");
            }
            if (dwExtStyle & WS_EX_NOPARENTNOTIFY) {
                dprintf(L"WS_EX_NOPARENTNOTIFY");
            }
            if (dwExtStyle & WS_EX_TOPMOST) {
                dprintf(L"WS_EX_TOPMOST");
            }
            if (dwExtStyle & WS_EX_ACCEPTFILES) {
                dprintf(L"WS_EX_ACCEPTFILES");
            }
            if (dwExtStyle & WS_EX_TRANSPARENT) {
                dprintf(L"WS_EX_TRANSPARENT");
            }
            if (dwExtStyle & WS_EX_MDICHILD) {
                dprintf(L"WS_EX_MDICHILD");
            }
            if (dwExtStyle & WS_EX_TOOLWINDOW) {
                dprintf(L"WS_EX_TOOLWINDOW");
            }
            if (dwExtStyle & WS_EX_WINDOWEDGE) {
                dprintf(L"WS_EX_WINDOWEDGE");
            }
            if (dwExtStyle & WS_EX_CLIENTEDGE) {
                dprintf(L"WS_EX_CLIENTEDGE");
            }
            if (dwExtStyle & WS_EX_CONTEXTHELP) {
                dprintf(L"WS_EX_CONTEXTHELP");
            }
            if (dwExtStyle & WS_EX_RIGHT) {
                dprintf(L"WS_EX_RIGHT");
            }
            if (dwExtStyle & WS_EX_LEFT) {
                dprintf(L"WS_EX_LEFT");
            }
            if (dwExtStyle & WS_EX_RTLREADING) {
                dprintf(L"WS_EX_RTLREADING");
            }
            if (dwExtStyle & WS_EX_LTRREADING) {
                dprintf(L"WS_EX_LTRREADING");
            }
            if (dwExtStyle & WS_EX_LEFTSCROLLBAR) {
                dprintf(L"WS_EX_LEFTSCROLLBAR");
            }
            if (dwExtStyle & WS_EX_RIGHTSCROLLBAR) {
                dprintf(L"WS_EX_RIGHTSCROLLBAR");
            }
            if (dwExtStyle & WS_EX_CONTROLPARENT) {
                dprintf(L"WS_EX_CONTROLPARENT");
            }
            if (dwExtStyle & WS_EX_STATICEDGE) {
                dprintf(L"WS_EX_STATICEDGE");
            }
            if (dwExtStyle & WS_EX_APPWINDOW) {
                dprintf(L"WS_EX_APPWINDOW");
            }
            if (dwExtStyle & WS_EX_LAYERED) {
                dprintf(L"WS_EX_LAYERED");
            }
            if (dwExtStyle & WS_EX_NOINHERITLAYOUT) {
                dprintf(L"WS_EX_NOINHERITLAYOUT");
            }
            if (dwExtStyle & WS_EX_LAYOUTRTL) {
                dprintf(L"WS_EX_LAYOUTRTL");
            }
            if (dwExtStyle & WS_EX_COMPOSITED) {
                dprintf(L"WS_EX_COMPOSITED");
            }
            if (dwExtStyle & WS_EX_NOACTIVATE) {
                dprintf(L"WS_EX_NOACTIVATE");
            }
